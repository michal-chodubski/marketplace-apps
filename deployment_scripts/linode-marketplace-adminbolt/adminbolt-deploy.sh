...
export WORK_DIR="/root/marketplace-apps" # moved to root dir because adminbolt install reshapes system heavily
export MARKETPLACE_APP="apps/linode-marketplace-adminbolt"
...
# adminbolt installer supports AlmaLinux 9 only
if [ -f /etc/os-release ]; then
    . /etc/os-release

    if [ "$ID" != "almalinux" ]; then
        echo "[error] adminbolt supports AlmaLinux 9 only. Detected: $ID $VERSION_ID"
        exit 1
    fi

    if [[ ! "$VERSION_ID" =~ ^9(\.[0-9]+)?$ ]]; then
        echo "[error] adminbolt supports AlmaLinux 9 only. Detected version: $VERSION_ID"
        exit 1
    fi
else
